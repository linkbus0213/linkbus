...
function Storefront() {
  ...
  const [brand, setBrand] = useState<Brand | '전체'>('전체')
  ...
  const filtered = useMemo(() => phones.filter((phone) => (brand === '전체' || phone.brand === brand) && (brand === '전체' || phone.series === series)), [phones, brand, series])
  function chooseBrand(next: Brand | '전체') { setBrand(next); setSeries(next === '삼성' ? samsungSeries[0] : appleSeries[0]) }
  return <>
    <header className="topbar"><div className="notice-ticker"><div className="ticker-track">{['LINKBUS 휴대폰 구매 상담 OPEN', '전 통신사 조건 비교', '최대할인구매 상품 확인', '안전한 개통 절차 안내', 'LINKBUS 휴대폰 구매 상담 OPEN', '전 통신사 조건 비교', '최대할인구매 상품 확인', '안전한 개통 절차 안내'].map((text, i) => <span key={`${text}-${i}`}>{text}</span>)}</div></div><div className="top-inner"><a className="brand-logo" href="#top"><span>LINK</span>BUS</a></div><nav className="menu-line">{['애플', '삼성', '기타', '유심전용', '인터넷/IPTV', '리뷰', '질문답변', '이벤트', '고객센터'].map((item) => <a key={item} href="#popular">{item}</a>)}</nav></header>
    <main id="top">
      ...
      <section id="popular" className="popular-section new-phone-section"><div className="section-title"><h2>최대할인구매</h2></div><div className="brand-tabs">{brands.map((item) => <button key={item} className={brand === item ? 'active' : ''} onClick={() => chooseBrand(item)}>{item === '애플' ? '애플 | APPLE' : item === '삼성' ? '삼성 | SAMSUNG' : item}</button>)}</div>{brand !== '전체' && <div className="series-tabs">{seriesList.map((item) => <button key={item} className={series === item ? 'active' : ''} onClick={() => setSeries(item)}>{item}</button>)}</div>}<div className="phone-strip new-phone-grid">{filtered.map((phone) => <ProductTile key={phone.id} phone={phone} />)}{!filtered.length && <p className="empty">조건에 맞는 상품이 없습니다.</p>}</div></section>
    </main><SiteFooter /></>
}
