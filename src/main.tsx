...
function Storefront({ page = 'home' }: { page?: StorePage }) {
  ...
  const filtered = useMemo(() => sortPhonesForDisplay(phones.filter((phone) => { const pageBrand = page === 'apple' ? '애플' : page === 'samsung' ? '삼성' : page === 'kids' ? '기타' : null; const activeBrand = pageBrand || (brand === '전체' ? null : brand); const isBrandAll = !activeBrand; const isSeriesAll = series === '애플 제품 전체' || series === '삼성 제품 전체' || page === 'kids'; const brandOk = activeBrand ? phone.brand === activeBrand : true; const seriesOk = isBrandAll || isSeriesAll || phone.series === series; const simOk = page === 'sim' ? (phone.carrier === '알뜰폰' || phone.tag?.includes('유심') || phone.subtitle?.includes('유심')) : true; return brandOk && seriesOk && simOk })), [phones, brand, series, page])
  function chooseBrand(next: Brand | '전체') { setBrand(next); setSeries(next === '삼성' ? samsungSeries[0] : next === '애플' ? appleSeries[0] : appleSeries[0]) }
