...
const cautionSections = [
  ...
  { title: '교환/환불 안내', items: ['개통 또는 박스 개봉 이후에는 고객 단순변심에 의한 청약철회·교환·반품·환불이 불가합니다.', '단말기 수령 기간과 관계 없이 개통하거나 개봉한 이후에는 단말기에 대한 청약철회가 불가합니다.', '휴대폰 수령 후 포장박스 개봉, 구성품 사용·누락, 상품 훼손, 고객 부주의로 인한 파손·침수·분실이 있으면 교환·반품·환불이 불가합니다.', '교환·반품 접수 시 단말기 박스, 구성품, 사은품 등 수령한 모든 구성품을 함께 반납해야 합니다. 구성품이 누락되면 처리가 불가하거나 비용이 청구됩니다.', '제품 하자로 교환·반품을 요청하는 경우 제조사 서비스센터의 불량판정서, 교품증, 통화품질확인서 등 공식 판정 서류가 필요합니다.', '[삼성/기타 제조사] 개통일 포함 D+14일 이내 제조사 불량판정서 또는 통화품질확인서가 있어야 교환·반품 접수가 가능합니다.', '[삼성/기타 제조사] 개통일 포함 D+14일 이후 기기 불량은 제조사 AS센터를 통한 수리 또는 제조사 기준에 따른 조치만 가능합니다.', '[애플 교환] 개통일 포함 D+30일 이내 Apple 공인서비스센터 판단 기준에 따라 새 제품 교환이 가능합니다.', '[애플 교환] 개통일 포함 D+30일 이후에는 Apple 공인서비스센터 기준에 따라 서비스 제품 교환 또는 부분 수리만 가능하며 새 제품 교환은 불가합니다.', '[애플 반품] 개통일 포함 D+9일 이내 단말기 불량 건에 한해 Apple 공인서비스센터의 작업인가서 등 공식 확인 서류가 있어야 반품 접수가 가능합니다.', '[애플 통화품질 반품] 개통일 포함 D+14일 이내 통신사 통화품질 불량 판정이 있는 경우에만 반품 접수가 가능합니다.', 'Apple 제품의 기능·외관 결함 여부와 AS 가능 여부는 Apple 공인서비스센터 판단 기준을 따릅니다.', '제품 불량으로 반품하는 경우 구글락, 아이클라우드락, 화면잠금, 나의 iPhone 찾기 등 계정 잠금을 반드시 해제해야 합니다. 잠금이 해제되지 않으면 반품 처리가 불가합니다.', '최초 구매 시 별도 판매조건, 사전 고지, 상품 상세 안내에 동의한 경우 해당 조건이 우선 적용됩니다.'] },
]
function CautionText({ text: _text }: { text: string }) {
  ...
  const exchangeIntro = section.items.slice(0, 4)
  const exchangeTail = section.items.slice(10)
  return <div className="notice-clean"><div className="notice-clean-head"><div><h3>유의사항</h3><p>구매 전 꼭 확인해야 하는 내용을 항목별로 정리했습니다.</p></div><span>LINKBUS 안내</span></div><div className="notice-clean-tabs">{cautionSections.map((item, index) => <button type="button" key={item.title} className={active === index ? 'active' : ''} onClick={() => setActive(index)}>{item.title}</button>)}</div><article className="notice-clean-panel"><h4>{section.title}</h4>{isExchange ? <><ul>{exchangeIntro.map((item) => <li key={item}><HighlightText text={item} /></li>)}</ul><div className="exchange-rule-boxes"><section><h5>삼성/기타 제조사</h5><dl><dt>교환·반품 처리기간</dt><dd>개통일 포함 D+14일 이내</dd><dt>필수 서류</dt><dd>제조사 불량판정서 또는 통화품질확인서</dd><dt>D+14일 이후</dt><dd>제조사 AS센터 기준에 따른 수리 또는 조치만 가능</dd></dl></section><section><h5>애플</h5><dl><dt>교환</dt><dd>D+30일 이내 Apple 공인서비스센터에서 새 제품 교환 가능<br/>D+30일 이후 서비스 제품 교환 또는 부분 수리만 가능</dd><dt>반품</dt><dd>D+9일 이내 단말기 불량 건에 한해 Apple 공인서비스센터 작업인가서 등 공식 확인 서류 필요</dd><dt>통화품질 반품</dt><dd>D+14일 이내 통신사 통화품질 불량 판정이 있는 경우에만 접수 가능</dd></dl></section></div><ul>{exchangeTail.map((item) => <li key={item}><HighlightText text={item} /></li>)}</ul></> : <ul>{section.items.map((item) => <li key={item}><HighlightText text={item} /></li>)}</ul>}</article></div>
}
