...
type StorageOption = { label: string; price?: number | null; support?: number | null; rebate?: number | null; supportByCarrier?: CarrierMoney; rebateByCarrier?: CarrierMoney; rebateByCarrierJoin?: RebateByCarrierJoin }
type Phone = {
...
function rebateValue(storage: StorageOption | undefined, carrier: CarrierKey, joinType: JoinType, fallback?: number | null) { return storage?.rebateByCarrierJoin?.[carrier]?.[joinType] ?? storage?.rebateByCarrier?.[carrier] ?? storage?.rebate ?? fallback ?? 0 }
...
function cleanStorages(storages?: StorageOption[] | null) { return storages?.length ? storages.filter(s => s.label).map(s => ({ label: s.label, price: s.price ?? null, support: s.support ?? null, rebate: s.rebate ?? null, supportByCarrier: s.supportByCarrier || {}, rebateByCarrier: s.rebateByCarrier || {}, rebateByCarrierJoin: s.rebateByCarrierJoin || {} })) : defaultStorages }
...
function App() { return location.pathname.startsWith('/admin') ? <AdminApp /> : <Storefront /> }
...
function Storefront() {
  ...
  return <>
    ...
    <main id="top">
      ...
      <section id="popular" className="popular-section"><div className="section-title"><h2>인기 급상승 상품</h2><p>모델을 누르면 색상·용량·상세 조건을 확인할 수 있어요.</p></div><div className="brand-tabs">{brands.map((item) => <button key={item} className={brand === item ? 'active' : ''} onClick={() => chooseBrand(item)}>{item === '애플' ? '애플 | APPLE' : item === '삼성' ? '삼성 | SAMSUNG' : item}</button>)}</div>{brand !== '전체' && <div className="series-tabs">{seriesList.map((item) => <button key={item} className={series === item ? 'active' : ''} onClick={() => setSeries(item)}>{item}</button>)}</div>}<label className="search-pill"><Search size={17}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="모델명, 통신사, 가입유형 검색" /></label><div className="phone-strip">{filtered.map((phone) => <button className="product-tile" key={phone.id} onClick={() => setSelected(phone)}><span className="ribbon">{phone.badge}</span><figure><img src={phone.colors[0]?.image || phone.image} alt={phone.name}/></figure><b>{phone.name}</b><small>{phone.subtitle}</small><strong>{money(customerPrincipal(phone.storages[0]?.price ?? phone.price, carrierValue(phone.storages[0]?.supportByCarrier, normalizeCarrier(phone.carrier), phone.storages[0]?.support ?? phone.support), rebateValue(phone.storages[0], normalizeCarrier(phone.carrier), phone.joinType, phone.rebate)))}</strong></button>)}{!filtered.length && <p className="empty">조건에 맞는 상품이 없습니다.</p>}</div></section>
    ...
    </main><footer className="footer"><div className="footer-call"><h4>구매 문의 관련 상담</h4><strong>카카오톡 상담 / 전화 상담 준비중</strong><p>평일 10:00 ~ 18:00 · 토요일 10:00 ~ 15:00</p></div><div><b>LINKBUS</b><p>상호/사업자 정보/통신판매업 신고번호는 실제 사업자 가입 후 입력합니다.</p></div></footer>{selected && <ProductModal phone={selected} onClose={() => setSelected(null)} />}</>
}
...
function StorageEditor({ storages, onChange }: { storages: StorageOption[]; onChange: (v: StorageOption[]) => void }) {
  ...
  return <div className="option-editor"><div className="option-title"><b>저장공간 · 통신사별 지원금/가입유형별 리베이트</b><button type="button" onClick={() => onChange([...storages, { label: '256G', supportByCarrier: {}, rebateByCarrierJoin: {} }])}>용량 추가</button></div>{storages.map((s, i) => {
    ...
    return <div className="storage-edit-card" key={i}><div className="storage-edit-row"><input value={s.label} onChange={(e) => setStorage(i, { label: e.target.value })} placeholder="256G"/><input type="number" value={s.price ?? ''} onChange={(e) => setStorage(i, { price: e.target.value === '' ? null : Number(e.target.value) })} placeholder="출고가"/><input type="number" value={s.support ?? ''} onChange={(e) => setStorage(i, { support: e.target.value === '' ? null : Number(e.target.value) })} placeholder="기본 공통지원금"/><input type="number" value={s.rebate ?? ''} onChange={(e) => setStorage(i, { rebate: e.target.value === '' ? null : Number(e.target.value) })} placeholder="기본 리베이트"/><button type="button" onClick={() => onChange(storages.filter((_, n) => n !== i))}>삭제</button></div><div className="carrier-support-grid join-rebate-grid">{carrierKeys.map((carrier) => <div key={carrier}><b>{carrier}</b><input type="number" value={s.supportByCarrier?.[carrier] ?? ''} onChange={(e) => setCarrierMoney(i, 'supportByCarrier', carrier, e.target.value === '' ? null : Number(e.target.value))} placeholder="공통지원금"/><div className="join-rebate-fields">{joinTypes.map((type) => <label key={type}>{type}<input type="number" value={s.rebateByCarrierJoin?.[carrier]?.[type] ?? ''} onChange={(e) => setJoinRebate(i, carrier, type, e.target.value === '' ? null : Number(e.target.value))} placeholder="리베이트"/></label>)}</div><small>번호이동 고객가 {money(customerPrincipal(s.price, carrierValue(s.supportByCarrier, carrier, s.support), rebateValue(s, carrier, '번호이동', s.rebate)))}</small></div>)}</div><p className="formula-note">계산식: 출고가 - 공통지원금 - 가입유형별 리베이트 = 고객구매가격(할부원금). 예: SKT 번호이동 기준 {money(samplePrincipal)}</p></div> })}</div>
}
...
function ProductModal({ phone, onClose }: { phone: Phone; onClose: () => void }) {
  ...
  const [colorIndex, setColorIndex] = useState(0), [storageIndex, setStorageIndex] = useState(0), [targetCarrier, setTargetCarrier] = useState(defaultCarrier), [joinType, setJoinType] = useState<JoinType>(phone.joinType)
  ...
  const support = carrierValue(storage?.supportByCarrier, targetCarrier as CarrierKey, storage?.support ?? phone.support)
  const rebate = rebateValue(storage, targetCarrier as CarrierKey, joinType, phone.rebate)
  ...
  function changeCarrier(next: CarrierKey) { const nextPlans = carrierPlans.filter((p) => p.carrier === next); setTargetCarrier(next); setPlanName((nextPlans[0] || carrierPlans[0]).name) }
  return <div className="modal-backdrop" onClick={onClose}><section className="detail-modal phonesawa-like" onClick={(e) => e.stopPropagation()}><button className="close" onClick={onClose}><X/></button><div className="detail-left"><div className="detail-phone-title"><h2>{phone.name}</h2><p>{storage?.label || ''} · {phone.subtitle}</p></div><div className="modal-media"><img src={color?.image || phone.image} alt={`${phone.name} ${color?.name || ''}`}/></div><div className="bill-card"><span>월별 청구금액</span><strong>{money(monthlyBill)}</strong><small>└ 단말 할부금 {money(installment.monthly)}<br/>└ 요금제 {money(plan.fee)}</small></div><div className="cost-grid"><div><b>휴대폰 할부금</b><strong>{money(installment.monthly)}</strong><small>고객구매가격/할부원금 {money(principal)}<br/>할부수수료 {money(installment.interest)}<br/>원리금균등 연 5.9% · 24개월</small></div><div><b>당월 예상금액</b><strong>{money(thisMonthDevice + thisMonthPlan)}</strong><small>단말 {money(thisMonthDevice)} + 요금 {money(thisMonthPlan)}<br/>오늘 기준 남은 일수 일할 계산</small></div></div></div><div className="detail-right"><div className="top-price"><span>월</span><strong>{money(monthlyBill)}</strong></div><div className="option-block"><h3>색상</h3><div className="color-choice-row">{phone.colors.map((c, i) => <button key={`${c.name}-${i}`} className={i === colorIndex ? 'active' : ''} onClick={() => setColorIndex(i)}><i style={{ background: c.hex }} />{c.name}</button>)}</div></div><div className="option-block"><h3>용량</h3><div className="storage-choice-row">{phone.storages.map((s, i) => <button key={`${s.label}-${i}`} className={i === storageIndex ? 'active' : ''} onClick={() => setStorageIndex(i)}>{s.label}</button>)}</div><div className="support-line"><span>출고가 : {money(devicePrice)}</span><span>공통지원금 : {money(support)}</span><span>{joinType} 리베이트 : {money(rebate)}</span><span>고객구매가격 : {money(principal)}</span></div></div><div className="carrier-box"><div><b>가입유형</b><select value={joinType} onChange={(e) => setJoinType(e.target.value as JoinType)}><option>번호이동</option><option>기기변경</option><option>신규가입</option></select></div><div><b>현재 통신사</b><select defaultValue="KT"><option>KT</option><option>SKT</option><option>LGU+</option><option>알뜰폰</option></select></div><div><b>가입 예정 통신사</b><select value={targetCarrier} onChange={(e) => changeCarrier(e.target.value as CarrierKey)}><option>SKT</option><option>KT</option><option>LGU+</option></select></div></div><div className="plan-row plan-picker"><b>요금제</b><select value={plan.name} onChange={(e) => setPlanName(e.target.value)}>{activePlans.map((p) => <option key={p.name}>{p.name}</option>)}</select><strong>{money(plan.fee)}</strong><small>{plan.data} · {plan.voice}</small></div><div className="benefit-box"><b>요금제 혜택</b>{plan.benefits.map((benefit) => <span key={benefit}>□ {benefit}</span>)}</div><div className="modal-actions"><a className="kakao-talk" href={kakaoHref(phone.name)}><MessageCircle size={19}/> 카카오톡 상담하기</a><a className="outline" href="#consult" onClick={onClose}>신청하기</a></div></div></section></div>
}
