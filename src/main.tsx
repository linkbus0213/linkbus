...
const BUSINESS_INFO = {
  ...
  customerCenter: '상담 신청 후 순차 안내',
  hours: '평일 10:00 ~ 18:00 · 토요일 10:00 ~ 15:00',
...
function HeaderNav() { return <header className="topbar"><div className="top-inner"><a className="brand-logo" href="/"><span>LINK</span>BUS</a></div><nav className="menu-line"><a href="/#popular">상품목록</a><a href="/#consult">상담신청</a><a href="/business-info">사업자정보</a><a href="/terms">이용약관</a><a href="/privacy">개인정보처리방침</a><a href="/refund">교환/반품/환불</a></nav></header> }
function SiteFooter() { return <footer className="footer"><div className="footer-call"><h4>구매 문의 관련 상담</h4><strong>카카오톡 상담 / 전화 상담 준비중</strong><p>{BUSINESS_INFO.hours}</p><p>고객센터: {BUSINESS_INFO.customerCenter}</p></div><div className="footer-business"><b>LINKBUS</b><p><span>상호: {BUSINESS_INFO.name}</span><span>대표자: {BUSINESS_INFO.representative}</span><span>사업자등록번호: {BUSINESS_INFO.businessNumber}</span><span>통신판매업신고번호: {BUSINESS_INFO.mailOrderNumber}</span><span>신고기관: {BUSINESS_INFO.mailOrderAgency}</span><span>주소: {BUSINESS_INFO.address}</span><span>이메일: {BUSINESS_INFO.email}</span><span>호스팅서비스 제공자: {BUSINESS_INFO.hostingProvider}</span><span>개인정보보호책임자: {BUSINESS_INFO.privacyManager}</span></p><div className="footer-links"><a href="/business-info">사업자정보</a><a href={BUSINESS_INFO.ftcUrl} target="_blank" rel="noreferrer">공정위 사업자정보확인</a><a href="/terms">이용약관</a><a href="/privacy">개인정보처리방침</a><a href="/refund">교환/반품/환불</a></div><IctCertBadge /></div></footer> }
...
function BusinessInfoContent() { return <div className="legal-content"><h2>사업자 정보</h2><dl className="business-dl"><dt>상호</dt><dd>{BUSINESS_INFO.name}</dd><dt>대표자</dt><dd>{BUSINESS_INFO.representative}</dd><dt>사업자등록번호</dt><dd>{BUSINESS_INFO.businessNumber}</dd><dt>통신판매업신고번호</dt><dd>{BUSINESS_INFO.mailOrderNumber}</dd><dt>신고기관</dt><dd>{BUSINESS_INFO.mailOrderAgency}</dd><dt>사업장 소재지</dt><dd>{BUSINESS_INFO.address}</dd><dt>업태</dt><dd>{BUSINESS_INFO.businessType}</dd><dt>종목</dt><dd>{BUSINESS_INFO.businessItem}</dd><dt>고객센터</dt><dd>{BUSINESS_INFO.customerCenter}</dd><dt>이메일</dt><dd>{BUSINESS_INFO.email}</dd><dt>호스팅서비스 제공자</dt><dd>{BUSINESS_INFO.hostingProvider}</dd><dt>개인정보보호책임자</dt><dd>{BUSINESS_INFO.privacyManager}</dd></dl><p className="legal-note">사업자 정보는 사업자등록증 및 통신판매업신고증 기준으로 표시합니다.</p><p><a className="legal-button" href={BUSINESS_INFO.ftcUrl} target="_blank" rel="noreferrer">공정거래위원회 사업자정보 확인</a></p><IctCertBadge /></div> }
function TermsContent() { return <div className="legal-content"><h2>제1조 목적</h2><p>본 약관은 {BUSINESS_INFO.name}가 운영하는 LINKBUS 웹사이트에서 제공하는 휴대폰 상담, 구매 조건 안내, 개통 신청 안내 등 서비스의 이용 조건과 절차, 권리·의무를 정합니다.</p><h2>제2조 서비스의 성격</h2><p>LINKBUS는 휴대폰 단말기, 요금제, 가입유형, 지원금 및 구매 조건을 안내하고 상담 신청을 접수합니다. 실제 개통 가능 여부와 최종 조건은 통신사 정책, 재고, 신용·회선 상태, 해피콜 및 공식 신청서 확인 결과에 따라 달라질 수 있습니다.</p><h2>제3조 약관의 게시 및 변경</h2><p>본 약관은 웹사이트에 게시하며, 변경 시 적용일자와 주요 변경 내용을 공지합니다. 소비자에게 불리한 변경은 충분한 사전 고지 기간을 둡니다.</p><h2>제4조 구매 신청 및 계약 성립</h2><p>이용자는 상담 신청 또는 별도 신청 절차를 통해 구매 의사를 표시할 수 있습니다. 계약은 재고, 가입조건, 본인확인, 통신사 심사 및 개통 가능 여부가 확인된 뒤 성립합니다.</p><h2>제5조 가격·지원금·요금 안내</h2><p>표시된 금액은 출고가, 공통지원금, 추가지원금/할인, 요금제, 약정기간, 할부기간 등 조건을 전제로 산정됩니다. 정책 변경이나 재고 변동에 따라 안내 금액은 달라질 수 있으며, 변경 시 개통 전 안내합니다.</p><h2>제6조 배송 및 개통</h2><p>배송 및 개통 절차는 상품 상세페이지와 상담 안내에 따릅니다. 번호이동, 기기변경, 신규가입 여부에 따라 개통 순서와 기존 회선 이용 가능 여부가 달라질 수 있습니다.</p><h2>제7조 청약철회·교환·반품</h2><p>청약철회, 교환, 반품, 환불은 전자상거래 관련 법령, 통신사 개통 정책, 제조사 불량 판정 기준, 상품 상세 안내 및 <a href="/refund">교환·반품·환불 안내</a>에 따릅니다.</p><h2>제8조 이용자의 의무</h2><p>이용자는 본인 명의의 정확한 정보를 제공해야 하며, 명의도용, 허위 신청, 비정상 사용, 불법 목적의 개통을 해서는 안 됩니다.</p><h2>제9조 개인정보 보호</h2><p>개인정보는 <a href="/privacy">개인정보처리방침</a>에 따라 처리합니다.</p><h2>제10조 사업자 정보</h2><p>사업자 정보는 <a href="/business-info">사업자정보</a> 페이지와 사이트 하단에서 확인할 수 있습니다.</p><h2>제11조 분쟁 해결</h2><p>분쟁은 관련 법령과 상호 협의를 통해 해결하며, 필요한 경우 소비자분쟁해결기준 및 관할 기관의 절차를 따릅니다.</p><p className="legal-note">시행일: 2026년 5월 20일</p></div> }
function PrivacyContent() { return <div className="legal-content"><h2>1. 개인정보 처리 목적</h2><p>LINKBUS는 상담 신청 접수, 본인 확인, 희망 모델·요금제 안내, 구매 및 개통 절차 안내, 고객 문의 응대, 분쟁 대응을 위해 개인정보를 처리합니다.</p><h2>2. 수집 항목</h2><ul><li>필수: 이름, 연락처, 희망 모델, 가입유형, 상담 내용</li><li>개통 진행 시 추가 수집: 통신사 신청서 작성 및 본인확인에 필요한 정보. 해당 정보는 공식 신청 절차에서 별도 안내 및 동의를 받습니다.</li><li>자동 수집 가능 항목: 접속 로그, 쿠키, 기기·브라우저 정보</li></ul><h2>3. 보유 및 이용 기간</h2><p>상담 신청 정보는 상담 완료 및 분쟁 대응을 위해 수집일로부터 최대 3년간 보관 후 파기합니다. 관계 법령상 보존 의무가 있는 경우 해당 기간 동안 보관합니다.</p><h2>4. 제3자 제공</h2><p>LINKBUS는 법령에 근거하거나 이용자 동의가 있는 경우를 제외하고 개인정보를 제3자에게 제공하지 않습니다. 통신사 개통 신청이 필요한 경우 이용자의 별도 동의 후 해당 통신사 또는 공식 신청 시스템에 필요한 정보가 제공될 수 있습니다.</p><h2>5. 처리 위탁</h2><p>서비스 운영을 위해 호스팅, 데이터 저장, 상담 접수, 문자·알림, 배송 등 업무를 외부 서비스에 위탁할 수 있으며, 위탁 발생 시 필요한 범위 내에서 안전하게 관리합니다.</p><h2>6. 파기 절차 및 방법</h2><p>보유기간 경과 또는 처리 목적 달성 시 전자적 파일은 복구 불가능한 방법으로 삭제하고, 출력물은 분쇄 또는 파쇄합니다.</p><h2>7. 정보주체의 권리</h2><p>이용자는 개인정보 열람, 정정, 삭제, 처리정지, 동의 철회를 요청할 수 있습니다. 요청은 고객센터 또는 개인정보보호책임자 연락처로 접수합니다.</p><h2>8. 쿠키의 설치·운영 및 거부</h2><p>사이트는 서비스 품질 개선과 접속 통계 확인을 위해 쿠키를 사용할 수 있습니다. 이용자는 브라우저 설정에서 쿠키 저장을 거부할 수 있습니다.</p><h2>9. 안전성 확보 조치</h2><p>LINKBUS는 접근 권한 관리, 암호화 통신, 보안 설정, 개인정보 취급자 최소화 등 필요한 보호 조치를 시행합니다.</p><h2>10. 개인정보보호책임자</h2><dl className="business-dl compact"><dt>책임자</dt><dd>{BUSINESS_INFO.privacyManager}</dd><dt>이메일</dt><dd>{BUSINESS_INFO.email}</dd><dt>고객센터</dt><dd>{BUSINESS_INFO.customerCenter}</dd></dl><p className="legal-note">시행일: 2026년 5월 20일</p></div> }
function RefundContent() { return <div className="legal-content"><h2>교환·반품·환불 기본 원칙</h2><p>휴대폰 단말기는 통신 개통, 박스 개봉, 제조사 불량 판정 여부에 따라 교환·반품 가능 범위가 달라집니다. 단순 변심은 미개봉·미개통 상태에서만 가능하며, 왕복 배송비가 발생할 수 있습니다.</p><h2>단말기 박스 개봉</h2><p>단말기 박스 개봉 시 개통 전이라도 단순 변심에 의한 교환·반품이 제한됩니다. 개통 불가 사유가 뒤늦게 발견된 경우에도 개봉 상태라면 출고가 납부 등 비용이 발생할 수 있습니다.</p><h2>불량 교환</h2><p>단말기 불량은 제조사 서비스센터의 불량 확인서 또는 교품증 등 공식 판정 서류 기준으로 처리합니다. 삼성, 애플 등 제조사별 기준과 처리 가능 기간은 다를 수 있습니다.</p><h2>개통 취소</h2><p>통화품질 불량 등 개통 취소 사유는 통신사 점검 및 규정에 따라 처리합니다. 고객 요청만으로 즉시 취소되지 않을 수 있습니다.</p><h2>배송</h2><p>배송은 상담 및 신청 절차에서 안내한 방식에 따르며, 지역 택배 상황에 따라 지연될 수 있습니다.</p><p className="legal-note">상세 조건은 상품 상세페이지의 구매 전 주의사항 및 상담 안내가 우선 적용됩니다.</p></div> }

function App() { if (location.pathname.startsWith('/admin')) return <AdminApp />; if (location.pathname.startsWith('/product/')) return <ProductPage />; if (location.pathname === '/terms') return <LegalPage kind="terms" />; if (location.pathname === '/privacy') return <LegalPage kind="privacy" />; if (location.pathname === '/refund') return <LegalPage kind="refund" />; if (location.pathname === '/business-info') return <LegalPage kind="business" />; return <Storefront /> }
...
function Storefront() {
  ...
  return <>
    ...
    <main id="top">
      <section className="hero-slider main-image-hero"><a className="main-banner-image" href="#popular" aria-label="LINKBUS 메인 배너"><img src="/images/linkbus-main-banner.png" alt="LINKBUS 메인 배너" /></a><div className="brand-shortcut-row"><button type="button" className="brand-shortcut apple" onClick={() => { chooseBrand('애플'); document.getElementById('popular')?.scrollIntoView({ behavior: 'smooth' }) }}><span>APPLE</span><b>애플 제품 바로가기</b><img src={officialImages.iphonePro} alt="애플 제품" /></button><button type="button" className="brand-shortcut samsung" onClick={() => { chooseBrand('삼성'); document.getElementById('popular')?.scrollIntoView({ behavior: 'smooth' }) }}><span>SAMSUNG</span><b>삼성 제품 바로가기</b><img src={officialImages.galaxy} alt="삼성 제품" /></button></div></section>
...
function ProductPage() {
  ...
  return <><HeaderNav /><main className="product-detail-page">{loading ? <p className="empty">상품 정보를 불러오는 중입니다.</p> : phone ? <ProductDetail phone={phone} /> : <section className="popular-section"><p className="empty">상품을 찾을 수 없습니다.</p><a className="outline page-back" href="/">목록으로 돌아가기</a></section>}</main><SiteFooter /></>
}
