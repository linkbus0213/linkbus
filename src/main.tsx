...
function Storefront({ page = 'home' }: { page?: StorePage }) {
  ...
  const [series, setSeries] = useState('아이폰 17 시리즈')
  ...
  React.useEffect(() => { const title = page === 'apple' ? '애플 최대할인구매' : page === 'samsung' ? '삼성 최대할인구매' : page === 'sim' ? '유심만개통' : page === 'used' ? '중고폰판매' : page === 'reviews' ? '리얼후기' : page === 'customer' ? '고객센터' : '휴대폰 구매 상담'; document.title = `링크버스 | ${title}` }, [page])
  const seriesList = brand === '삼성' ? samsungSeries : appleSeries
  const filtered = useMemo(() => sortPhonesForDisplay(phones.filter((phone) => { const pageBrand = page === 'apple' ? '애플' : page === 'samsung' ? '삼성' : null; const activeBrand = pageBrand || (brand === '전체' ? null : brand); const isBrandAll = !activeBrand; const isSeriesAll = series === '애플 제품 전체' || series === '삼성 제품 전체'; const brandOk = activeBrand ? phone.brand === activeBrand : true; const seriesOk = isBrandAll || isSeriesAll || phone.series === series; const simOk = page === 'sim' ? (phone.carrier === '알뜰폰' || phone.tag?.includes('유심') || phone.subtitle?.includes('유심')) : true; return brandOk && seriesOk && simOk })), [phones, brand, series, page])
  function chooseBrand(next: Brand | '전체') { setBrand(next); setSeries(next === '삼성' ? samsungSeries[0] : next === '애플' ? appleSeries[0] : appleSeries[0]) }
  const title = page === 'apple' ? '애플 최대할인구매' : page === 'samsung' ? '삼성 최대할인구매' : page === 'sim' ? '유심만개통' : page === 'used' ? '중고폰판매' : page === 'reviews' ? '리얼후기' : page === 'customer' ? '고객센터' : '최대할인구매'
  const subtitle = page === 'apple' ? '아이폰 모델별 출고가·할인·현재가격을 한눈에 확인하세요.' : page === 'samsung' ? '갤럭시 모델별 최대 할인 조건을 확인하세요.' : page === 'sim' ? '사용 중인 단말기는 그대로, 유심과 요금제만 상담합니다.' : page === 'used' ? '쓰던 휴대폰 판매 상담을 위한 전용 안내 페이지입니다.' : page === 'reviews' ? 'LINKBUS 이용 고객의 실제 후기를 모아 보여드릴 공간입니다.' : page === 'customer' ? '상담 가능 시간과 문의 방법을 안내드립니다.' : 'LINKBUS에서 준비한 휴대폰 최대 할인 구매 상품입니다.'
  return <><MainHeader active={page} /><main id="top">{page === 'home' ? <><section className="hero-slider main-image-hero"><a className="main-banner-image" href="#popular" aria-label="LINKBUS 메인 배너"><img src="/images/linkbus-main-banner.png" alt="LINKBUS 메인 배너" /></a><div className="brand-shortcut-row"><a className="brand-shortcut apple" href="/apple"><span>APPLE</span><b>애플 제품 바로가기</b><img src={officialImages.iphonePro} alt="애플 제품" /></a><a className="brand-shortcut samsung" href="/samsung"><span>SAMSUNG</span><b>삼성 제품 바로가기</b><img src={officialImages.galaxy} alt="삼성 제품" /></a></div></section><section id="popular" className="popular-section new-phone-section"><div className="section-title"><h2>최대할인구매</h2></div><div className="brand-tabs">{brands.map((item) => <button key={item} className={brand === item ? 'active' : ''} onClick={() => chooseBrand(item)}>{item === '애플' ? '애플 | APPLE' : item === '삼성' ? '삼성 | SAMSUNG' : item}</button>)}</div>{brand !== '전체' && <div className="series-tabs">{seriesList.map((item) => <button key={item} className={series === item ? 'active' : ''} onClick={() => setSeries(item)}>{item}</button>)}</div>}<div className="phone-strip new-phone-grid">{filtered.map((phone) => <ProductTile key={phone.id} phone={phone} />)}{!filtered.length && <p className="empty">등록된 상품이 없습니다. 상담으로 조건을 확인해 주세요.</p>}</div></section></> : <section className="standalone-page"><PageHero title={title} subtitle={subtitle} page={page} />{page === 'reviews' ? <ReviewsPage embedded /> : page === 'customer' ? <CustomerCenterPage embedded /> : page === 'used' ? <UsedPhonePage embedded /> : page === 'sim' ? <><ProductGrid phones={filtered} emptyText="유심만개통 상품은 준비 중입니다. 고객센터로 문의해 주세요." /><InfoGuide title="유심만개통 안내" items={['사용 중인 단말기는 그대로 두고 유심/요금제만 개통 상담을 진행합니다.', '통신사, 요금제, 번호이동/신규가입 가능 여부를 확인한 뒤 안내합니다.', '상담 신청 또는 고객센터로 연락 주시면 가능한 조건을 확인해드립니다.']} /></> : <><div className="series-tabs page-series-tabs">{(page === 'apple' ? appleSeries : page === 'samsung' ? samsungSeries : []).map((item) => <button key={item} className={series === item ? 'active' : ''} onClick={() => setSeries(item)}>{item}</button>)}</div><ProductGrid phones={filtered} /></>}</section>}</main><SiteFooter /></>
}
